...
output.path = "intermediate_data"
year = 2018

if (grid.level == "coarse") {
  grid.path = sprintf("%s/annual_WRF/M02_EnergyPlus_Forcing_Historical_LowRes_ann_%s/meta", input.path, year)
  output.name = sprintf("%s/building_to_wrf_grid.geojson", output.path)
  suf = ""
} else {
  grid.path = sprintf("%s/high res grid for reporting", input.path)
  output.name = sprintf("%s/building_to_finer_wrf_grid.geojson", output.path)
  suf = "_finer"
}

if (run.read.data) {
    df <- sf::st_read(sprintf("%s/compiled_LA_building.geojson", output.path))
...
la.boundary <- sf::read_sf(sprintf("%s/domain/la-county-boundary.geojson", input.path))
## verify ones not joined in qgis
## not.joined <- df %>%
##     dplyr::filter(!(OBJECTID %in% df.join$OBJECTID)) %>%
##     {.}
## not.joined %>%
##     sf::st_write("not_matched_to_grid_initial_s2_off.geojson")
...
dfs.to.tabulate.grid <- list(df.grid.no.geom, join.has.building, remove.not.modelled.type, remove.small.building, remove.no.height)
...
dfs.to.tabulate.building <- list(df.no.geom, join.has.building, remove.not.modelled.type, remove.small.building, remove.no.height)
...
tibble::tibble(steps = c("Before joining",
                         "Join grid with building footprint",
...
all.building.count.to.prototype.with.source %>%
  readr::write_csv(sprintf("%s/all_building_count_to_prototype_with_source%s.csv", output.path, suf))
```
...
join.type.divide %>%
  ...
  dplyr::summarise_if(is.numeric, tibble::lst(min, mean, ))
  summary()
...
join.type.divide.type.vin.cnt %>%
  ...
  dplyr::summarise(building.count = n()) %>%
  dplyr::ungroup() %>%
  readr::write_csv(sprintf("%s/summary/building_type_vintage_count_per_grid%s.csv", output.path, suf))
```
