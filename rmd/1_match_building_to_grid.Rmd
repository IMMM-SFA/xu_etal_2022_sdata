...
if (run.read.data) {
    df <- sf::st_read("~/Dropbox/workLBNL/CityBES/LA/data-raw/compiled_LA_building.geojson")
    df.centroid <- sf::st_centroid(df)
    df.grid <- sf::st_read("M02_EnergyPlus_Forcing_Historical_LowRes/meta/wrf-grids-origin.geojson")
    df.join <- df.grid %>%
        sf::st_join(df.centroid, join=sf::st_contains) %>%
        dplyr::filter(!is.na(OBJECTID)) %>%
        {.}
    df.join %>%
        sf::st_write("building_to_wrf_grid.geojson")
} else {
    df <- sf::st_read("~/Dropbox/workLBNL/CityBES/LA/data-raw/compiled_LA_building.geojson")
    df.grid <- sf::st_read("M02_EnergyPlus_Forcing_Historical_LowRes/meta/wrf-grids-origin.geojson")
    df.join = sf::st_read("building_to_wrf_grid.geojson")
}

la.boundary <- sf::read_sf("domain/la-county-boundary.geojson")
...
map.to.idf.name <- readr::read_csv("type_vintage_to_idf_mapping.csv") %>%
...
map.to.idf.name %>%
  distinct(idf.name) %>%
  readr::write_csv("unique_idf_used.csv")

idf.source = readr::read_csv("unique_idf_used_with_source.csv")
...
all.building.count.to.prototype.with.source %>%
  readr::write_csv("all_building_count_to_prototype_with_source.csv")
```
...
join.type.divide.numeric.summary %>%
  readr::write_csv("summary/numeric_field_stats_per_grid.csv")
```
...
join.type.divide.type.vin.cnt %>%
  readr::write_csv("compiled_cell_building.csv")
...
join.type.divide.type.vin.cnt %>%
  ...
  dplyr::summarise(building.count = n()) %>%
  dplyr::ungroup() %>%
  readr::write_csv("summary/building_type_vintage_count_per_grid.csv")
```
