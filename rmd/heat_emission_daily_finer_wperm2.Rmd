---
title: "2018 heat emission from EnergyPlus simulation at finer grid level"
author: "Yujie Xu"
date: "8/15/2022"
...
The following shows the average hourly heat emission for a weekday, a weekend day in summer and in winter

```{r plotAvgDiurnal, fig.width=15}
avg.emission.total <- to.plot.daily.total %>%
  dplyr::group_by(Date, day.status, season.status, Hour) %>%
  dplyr::summarise(value = mean(value)) %>%
  dplyr::ungroup() %>%
  {.}

avg.emission.total %>%
  ggplot2::ggplot(ggplot2::aes(x=Hour, y=value, group=interaction(day.status, season.status), color=day.status)) +
  ggplot2::geom_line() +
  ggplot2::ylab("Heat Emission (W/m2)") +
  ggplot2::theme()
```

```{r plotAvgDiurnalComp, fig.width=15}
avg.emission <- to.plot.daily %>%
  dplyr::group_by(variable, Date, day.status, season.status, Hour) %>%
  dplyr::summarise(value = mean(value)) %>%
  dplyr::ungroup() %>%
  {.}

avg.emission %>%
  ggplot2::ggplot(ggplot2::aes(x=Hour, y=value, group=interaction(day.status, season.status, variable), color=day.status)) +
  ggplot2::geom_line() +
  ggplot2::facet_grid(variable~season.status) +
  ggplot2::ylab("Heat Emission (W/m2)") +
  ggplot2::theme()
```

```{r checkDiurnalVariabilityByDay, fig.width=15, height=7}
...
to.plot.daily.total %>%
  ...
  ggplot2::coord_cartesian(ylim = c(-2, 17)) +
  ggplot2::facet_grid(season.status~day.status) +
  ggplot2::ggtitle("Total heat emission daily profile for typical summer and winter days") +
  ggplot2::ylab("Heat Emission (W/m2)") +
  ggplot2::theme()
...
to.plot.daily %>%
  ...
  ggplot2::ggplot(ggplot2::aes(Hour, value)) +
  ggplot2::geom_boxplot(outlier.shape = NA) +
  ggplot2::coord_cartesian(ylim = c(-2, 15)) +
