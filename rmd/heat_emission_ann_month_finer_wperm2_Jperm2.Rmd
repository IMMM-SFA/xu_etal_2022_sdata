...
get.annual.data <- function(time.pref, spatial.suf, df.area=NULL, unit="MJ/m2") {
  ...
  if (!is.null(df.area)) {
    ...
    if (unit=="W/m2") {
      heat.by.grid.annual.comp <- heat.by.grid.annual.comp %>%
        dplyr::mutate(emission.value = value * 0.000277778 / 8760 / area.m2) %>%
        {.}
    } else if (unit == "MJ/m2")
      heat.by.grid.annual.comp <- heat.by.grid.annual.comp %>%
        dplyr::mutate(emission.value = value * 1e-6 / area.m2) %>%
        {.}
  }
  heat.by.grid.annual.comp
}

get.monthly.data <- function(time.pref, spatial.suf, df.area=NULL, unit="MJ/m2") {
  heat.by.grid.monthly = readr::read_csv(sprintf("aggregated_heat/%s%s_monthly.csv", time.pref, spatial.suf))
  ...
  if (!is.null(df.area)) {
    ...
    if (unit == "W/m2") {
      heat.by.grid.monthly.comp <- heat.by.grid.monthly.comp %>%
        dplyr::mutate(emission.value = value * 0.000277778 / hours / area.m2) %>%
        {.}
    } else if (unit == "MJ/m2") {
      heat.by.grid.monthly.comp <- heat.by.grid.monthly.comp %>%
        dplyr::mutate(emission.value = value * 1e-6 / area.m2) %>%
...
get.top.k.percent.emission <- function(df, k) {
  df.high.emit <- df %>%
    dplyr::filter(emission.value > quantile(df$emission.value, probs = 1 - k / 100.0)) %>%
    {.}
...
heat.by.grid.annual.comp %>%
  ...
  dplyr::summarise_at(vars(emission.value),
                      tibble::lst(min, "Q1"=~quantile(., probs = 0.25), mean,
                                  median, "Q3"=~quantile(., probs = 0.75),
                                  max)) %>%
  dplyr::ungroup() %>%
  knitr::kable(caption = "Finer Grid Annual Heat Emission Summary Statistics (MJ/m2)", digits = 3)


heat.by.grid.annual.comp %>%
  dplyr::group_by(id) %>%
  dplyr::mutate(percentage = emission.GJ / sum(emission.GJ) * 100) %>%
  dplyr::ungroup() %>%
  dplyr::group_by(variable) %>%
  dplyr::summarise_at(vars(percentage),
                      tibble::lst(min, "Q1"=~quantile(., probs = 0.25), mean,
                                  median, "Q3"=~quantile(., probs = 0.75),
                                  max)) %>%
  dplyr::ungroup() %>%
  knitr::kable(caption = "Finer Grid Annual Heat Emission Percentage from Each Component", digits = 3) %>%
  {.}
```
...
```{r annHeatTotal, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
tm_shape(to.plot.ann.total) +
  tm_polygons(col="emission.value", n=10, style="quantile", midpoint=0, palette="seq", border.alpha = 0) +
  tm_layout(main.title = "2018 Annual Heat Emission (MJ/m2)",
...
```{r annHeatTotalLarge, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
...
```{r largeIDTypeComp, message=FALSE, eval=TRUE}
...
```{r annHeatByComponent, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
tm_shape(to.plot.ann.component) +
  tm_polygons(col="emission.value", n=10, style="quantile", midpoint=0, palette="seq", border.alpha = 0) +
  tm_facets(by="variable", ncol=3) +
  tm_layout(main.title = "Annual Heat Emission of the Five Component (MJ/m2)",
            legend.position=c("left", "bottom"), legend.outside=FALSE,
            aes.palette = list(seq = "-RdYlGn")) +
  tm_shape(la.boundary.valid) +
  tm_polygons("CITY", alpha=0, legend.show=FALSE)
...
```{r annSurfHeat, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
tm_shape(to.plot.ann.component %>% filter(variable == "emission.surf")) +
  tm_polygons(col="emission.value", n=10, style="quantile", midpoint=0, palette="seq", border.alpha = 0) +
  tm_layout(main.title = "Annual Building Surface Heat Emission (MJ/m2)",
            legend.position=c("left", "bottom"), legend.outside=FALSE,
            aes.palette = list(seq = "-RdYlGn")) +
  tm_shape(la.boundary.valid) +
  tm_polygons("CITY", alpha=0, legend.show=FALSE)
...
```{r annSurfHeatHigh, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
...
```{r largeSurfIDUseTypeComp, message=FALSE, eval=TRUE}
building.info %>%
...
```{r largeSurfIDTypeComp, message=FALSE, eval=TRUE}
building.info %>%
  dplyr::inner_join(top.k.percent.surf.id, by="id") %>%
  dplyr::group_by(building.type) %>%
  dplyr::summarise(building.area.m2 = sum(building.area.m2)) %>%
...
```{r annRejHeat, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
tm_shape(to.plot.ann.component %>% filter(variable == "emission.rej")) +
  tm_polygons(col="emission.value", n=10, style="quantile", midpoint=0, palette="seq", border.alpha = 0) +
  tm_layout(main.title = "Annual Building HVAC Rejection Heat Emission (MJ/m2)",
            legend.position=c("left", "bottom"), legend.outside=FALSE,
            aes.palette = list(seq = "-RdYlGn")) +
  tm_shape(la.boundary.valid) +
  tm_polygons("CITY", alpha=0, legend.show=FALSE)
...
```{r annRejHeatHigh, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
...
```{r largeRejIDUseTypeComp, message=FALSE, eval=TRUE}
building.info %>%
...
```{r largeRejIDTypeComp, message=FALSE, eval=TRUE}
building.info %>%
  dplyr::inner_join(top.k.percent.rej.id, by="id") %>%
  dplyr::group_by(building.type) %>%
  dplyr::summarise(building.area.m2 = sum(building.area.m2)) %>%
...
```{r monthly, message=FALSE, fig.width=8, fig.height=5}
...
heat.by.grid.monthly.comp %>%
  dplyr::filter(month %in% c("01", "04", "07")) %>%
  ...
  dplyr::summarise_at(vars(percentage),
                      tibble::lst(min, "Q1"=~quantile(., probs = 0.25), mean,
                                  ...
                                  max)) %>%
  dplyr::ungroup() %>%
  knitr::kable(caption = "Finer Grid Annual Heat Emission Percentage from Each Component", digits = 3, format="html") %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  {.}
...
monthly.avg.emission %>%
  ggplot2::ggplot(ggplot2::aes(x = month, y = emission.value)) +
  ggplot2::geom_point() +
  ggplot2::geom_line() +
...
```{r monthlyAvg, message=FALSE, fig.width=8, fig.height=5}
...
monthly.avg.emission.component %>%
  ggplot2::ggplot(ggplot2::aes(x = month, y = emission.value, color=variable)) +
  ggplot2::geom_point() +
  ggplot2::geom_line() +
  ggplot2::scale_x_continuous(breaks=1:12) +
  ggplot2::ylab(sprintf("Heat Emission (%s)", unit.of.measure)) +
...
```{r monthlyTotal, message=FALSE, fig.width=8, fig.height=6, eval=TRUE}
...
tm_shape(to.plot.jan.jul.total) +
  tm_polygons(col="emission.value", n=10, style="quantile", midpoint=0, palette="seq", border.alpha = 0) +
  tm_facets(by="month") +
  tm_layout(main.title = "Jan and July Heat Emission (MJ/m2)",
            legend.position=c("left", "bottom"), legend.outside=FALSE,
            aes.palette = list(seq = "-RdYlGn")) +
  tm_shape(la.boundary.valid) +
  tm_polygons("CITY", alpha=0, legend.show=FALSE)
...
```{r monthlyComp, message=FALSE, fig.width=8, fig.height=13, eval=TRUE}
...
```{r july, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
...
tm_shape(to.plot.jul.total) +
  tm_polygons(col="emission.value", n=10, style="quantile", midpoint=0, palette="seq", border.alpha = 0) +
  tm_facets(by="month") +
  tm_layout(main.title = "July Heat Emission (MJ/m2)",
            legend.position=c("left", "bottom"), legend.outside=FALSE,
            aes.palette = list(seq = "-RdYlGn")) +
  tm_shape(la.boundary.valid) +
  tm_polygons("CITY", alpha=0, legend.show=FALSE)
...
```{r julyTop, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
...
```{r julySurf, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
...
```{r julySurfTop, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
...
```{r julyRej, fig.width = 8, fig.height=5, eval=TRUE}
...
```{r julyRejTop, message=FALSE, fig.width=8, fig.height=5, eval=TRUE}
