...
result.by.type <- annual.sim.result %>%
...
heat.surf.xts = xts::xts(df$emission.surf, order.by=df$timestamp.pox)
heat.rej.xts = xts::xts(df$emission.rej, order.by=df$timestamp.pox)
heat.exfiltration.xts = xts::xts(df$emission.exfiltration, order.by=df$timestamp.pox)
heat.exhaust.xts = xts::xts(df$emission.exhaust, order.by=df$timestamp.pox)
heat.ref.xts = xts::xts(df$emission.ref, order.by=df$timestamp.pox)

dygraphs::dygraph(cbind(heat.exhaust.xts, heat.exfiltration.xts),
                  ## dygraphs::dygraph(cbind(heat.rej.xts, heat.exhaust.xts, heat.exfiltration.xts, heat.ref.xts),
                  main=sprintf("%s Heat Emission (excluding surface emission)", df$idf.kw[[1]])) %>%
...
annual.sim.EUI.avg %>%
  ...
  ggplot2::scale_fill_brewer(palette = "Paired", direction=-1) +
