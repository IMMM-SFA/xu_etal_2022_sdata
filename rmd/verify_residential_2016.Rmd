...
firstRun = FALSE
if (firstRun) {
  df.ann <- readr::read_csv("../annual_sim_result_by_idf_epw_2016.csv")
  ...
  df.sim.ann.eui <- df.ann %>%
    ...
    {.}
  df.sim.ann.eui %>%
    readr::write_csv("sim_ann_2016.csv")
} else {
  df.sim.ann.eui = readr::read_csv("sim_ann_2016.csv")
}
...
if (firstRun) {
  result.ann <- readr::read_csv("../annual_sim_result_by_idf_epw_2016.csv")
  annual.total.result <- result.ann %>%
    ...
    {.}
  annual.total.result %>%
    readr::write_csv("../annual_total_result_2016.csv")
} else {
  annual.total.result <- readr::read_csv("../annual_total_result_2016.csv")
}
...
df.cec = readr::read_csv("../CECdata/ElectricityByCounty_2016.csv")
...
total.ann.nb.use.cmp <- sim.ann.agg.to.nb.use.res %>%
  ...
  dplyr::mutate(data.source = "Simulation 2016") %>%
...
area.ratio.of.usetypes <- to.plot.total.ann.use.cmp %>%
  ...
  tidyr::gather(data.source, m2, `Energy Atlas 2016`:`Simulation 2016`) %>%
...
area.ratio.of.usetypes %>%
  ...
  dplyr::mutate(ratio.diff = `Simulation 2016` - `Energy Atlas 2016`) %>%
...
for (i in seq_along(usetypes)) {
  p1 <- to.plot.total.ann.use.tract.cmp %>%
    ...
    ggplot2::ggplot(ggplot2::aes(x=`Simulation 2016`, y=`Energy Atlas 2016`)) +
...
for (i in seq_along(usetypes)) {
  p1 <- to.plot.total.ann.use.tract.cmp %>%
    ...
    ggplot2::ggplot(ggplot2::aes(x=`Simulation 2016`, `Energy Atlas 2016`)) +
    ggplot2::geom_point(size = 0.2) +
    ## ggplot2::geom_smooth(method="lm", size = 0.5) +
    ggpmisc::stat_poly_line() +
    ggpmisc::stat_poly_eq(aes(label = paste(after_stat(eq.label),
