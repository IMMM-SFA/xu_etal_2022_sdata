---
title: "2018 heat emission from EnergyPlus simulation at coarse grid level normalized by building footprint"
author: "Yujie Xu"
date: "8/15/2022"
...
get.grid.building.size <- function(spatial.level) {
  if (spatial.level == "tract") {
    filename = "building_area_tract.csv"
  } else if (spatial.level == "finer"){
    filename = "building_area_grid_finer.csv"
  } else if (spatial.level == "coarse"){
    filename = "building_area_grid.csv"
  }
...
df.area <- get.grid.building.size("finer") %>%
  ## dplyr::rename(id = epw.id) %>%
  dplyr::mutate(area.m2 = footprint.area.m2) %>%
  dplyr::select(id, area.m2)
...
to.plot.daily.total %>%
  ...
  ggplot2::facet_grid(season.status~day.status) +
  ggplot2::coord_cartesian(ylim = c(-30, 300)) +
  ggplot2::ggtitle("Total heat emission daily profile for typical summer and winter days") +
  ggplot2::ylab("Heat Emission (W/m2)") +
  ggplot2::theme()
```
...
```{r plotDailyComp, message=FALSE, fig.width=15, fig.height=12}
to.plot.daily %>%
  dplyr::mutate(Hour = factor(Hour)) %>%
...
```{r plotDailyCompZoom, message=FALSE, fig.width=15, fig.height=12}
to.plot.daily %>%
  dplyr::mutate(Hour = factor(Hour)) %>%
  dplyr::mutate(day.label = paste0(season.status, " ", day.status)) %>%
  ggplot2::ggplot(ggplot2::aes(Hour, value)) +
...
```{r plotDailyCompZoomNonSurf, message=FALSE, fig.width=15, fig.height=12}
...
to.plot.daily %>%
  ...
  ggplot2::geom_boxplot(outlier.shape = NA) +
  ggplot2::facet_grid(variable~day.label) +
  ggplot2::coord_cartesian(ylim = c(-5, 35)) +
  ggplot2::ggtitle("Heat emission component daily profile for typical summer and winter days") +
  ggplot2::ylab("Heat Emission (W/m2)") +
  ggplot2::theme()
```
...
```{r hourlyAvgPerMonthZoom, message=FALSE, fig.width=15, fig.height=10}
df.hourly.avg.month %>%
  dplyr::mutate_at(vars(month, Hour), as.numeric) %>%
  ggplot2::ggplot(ggplot2::aes(x=Hour, y=value, group=interaction(month, Hour))) +
  ggplot2::geom_boxplot(outlier.shape = NA) +
...
The following shows the non-surface components

```{r hourlyAvgPerMonthCompZoomNonSurf, message=FALSE, fig.width=15, fig.height=8}
df.hourly.avg.month.component %>%
  dplyr::filter(variable != "emission.surf") %>%
  dplyr::mutate_at(vars(month, Hour), as.numeric) %>%
  ggplot2::ggplot(ggplot2::aes(x=Hour, y=value, group=interaction(month, Hour))) +
  ggplot2::geom_boxplot(outlier.shape = NA) +
  ggplot2::coord_cartesian(ylim = c(-5, 25)) +
  ggplot2::ylab("Heat Emission (W/m2)") +
  ggplot2::facet_grid(variable~month) +
  ggplot2::theme()
```
...
The following shows the non-surface component.

```{r hourAvgPerMonthCompSpaceNonSurf, message=FALSE, fig.width=15}
