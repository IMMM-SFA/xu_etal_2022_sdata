---
title: "2018 heat emission from EnergyPlus simulation at coarse grid level (12km x 12km)"
author: "Yujie Xu"
date: "8/15/2022"
...
The following shows the daily heat emission in the original scale. We can see a lot of large outliers. This indicates a large variability across different grid cells (12km x 12km). The total heat emission peaks around noon time.
...
```{r plotDailyComp, message=FALSE, fig.width=15, fig.height=10}
to.plot.daily %>%
  dplyr::mutate(Hour = factor(Hour)) %>%
...
```{r plotDailyCompNonSurf, message=FALSE, fig.width=15, fig.height=10}
to.plot.daily %>%
  dplyr::mutate(Hour = factor(Hour)) %>%
  dplyr::mutate(day.label = paste0(season.status, " ", day.status)) %>%
...
```{r plotDailyCompNonSurfZoom, message=FALSE, fig.width=15, fig.height=10}
to.plot.daily %>%
  dplyr::mutate(Hour = factor(Hour)) %>%
  dplyr::mutate(day.label = paste0(season.status, " ", day.status)) %>%
  dplyr::filter(variable != "emission.surf") %>%
  ggplot2::ggplot(ggplot2::aes(Hour, value)) +
  ggplot2::geom_boxplot(outlier.shape = NA) +
  ggplot2::coord_cartesian(ylim = c(-5000, 22000)) +
  ggplot2::facet_grid(variable~day.label) +
  ggplot2::ggtitle("Total heat emission daily profile for a summer day and a winter day") +
  ggplot2::ylab("Heat Emission (kWh)") +
  ggplot2::theme()
